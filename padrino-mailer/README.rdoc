...
Padrino also supports more structured mailers. We can define a custom mailer using a mailer block.
  ...
  MyAppName.mailers :sample do
    message :registration_email do |name, user_email_address|
      from 'admin@site.com'
      ...
      template 'path/to/render'  # optional, defaults to views/mailers/sample/registration_email.erb
    end
  end
...
Note that Padrino also supports an (older) ActionMailer-inspired syntax. The mailer also supports the attachment
of files and various other options. Be sure to check out the 

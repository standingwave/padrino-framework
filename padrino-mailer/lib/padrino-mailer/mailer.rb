module Padrino
  module Mailer
    ##
    # This is a verbatim copy of http://github.com/mikel/mail/blob/master/lib/mail/mail.rb
    #
    # Since the author use <tt>Mail.new, Mail.defaults etc...</tt> we cannot include/extend the module itself.
    #
    class << self
      ##
      ...
      #   Padrino::Mailer.new(string)
      #
      # Or creating via a block:
      #
      #   message = Padrino::Mailer.new do
      ...
      #
      #   message = Padrino::Mailer.new({:to => 'mikel@test.lindsaar.net',
      #                                  :from => 'bob@test.lindsaar.net',
      #                                  :subject => 'This is an email',
      #                                  :body => 'This is the body' })
      #
      ...
      #
      def new(*args, &block)
        Padrino::Mailer::Message.new(args, &block)
      end
      ...
      #   end
      #
      #   Padrino::Mailer.delivery_method.new  #=> Mail::SMTP instance
      #   Padrino::Mailer.retriever_method.new #=> Mail::POP3 instance
      #
      # Each mail object inherits the default set in Padrino::Mailer.delivery_method, however, on
      ...
      # configuration first before you use Padrino::Mailer.deliver, if you don't, an appropriate
      ...
      # You can also do:
      #
      #  mail = Padrino::Mailer.read('email.eml')
      #  mail.deliver!
      #
      # And your email object will be created and sent.
      #
      def deliver(*args, &block)
        mail = Padrino::Mailer.new(args, &block)
        mail.deliver
        mail
      end
      ...
      #
      def read(filename)
        Padrino::Mailer.new(File.read(filename))
      end
      ...
      def inform_observers(mail)
        @@delivery_notification_observers.each do |observer|
          observer.delivered_email(mail)
        end
      end

      def inform_interceptors(mail)
